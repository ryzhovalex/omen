...
class Assembler(Helper):
    ...
    @staticmethod
    @logger.head_catch
    def create_app(build: Build, config_cells: List[ConfigCell] = None, db_uri: str = None, module_path: str = None) -> Flask:
        ...
        assembler = Assembler(build=build)

        if module_path:
            assembler.module_path = module_path

        if config_cells:
            # Reassign existing cells to given ones. 
            for cell in config_cells:
                assembler.config_cells_by_name[cell.name] = cell
